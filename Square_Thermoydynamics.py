...
def main():
    ...
    probs_Arr = np.zeros((J_n, B_n, T_n, 4))
    ...
    for j in range(len(J_dom)):
        for b in range(len(B_dom)):
            for t in range(len(T_dom)):
                ...
                Ox_Arr[j, b, t] = np.max(O.x)
                Oy_Arr[j, b, t] = np.min(O.x)
    ...
    ### CREATE CSV
    with open("/home/twiggy/Research/Chiral_Phonon_Zeeman_Splitting/Data/FourLevel_Thermodynamics.csv", "w") as file:
        file.write("J, B, T, Ox, Oy, p1, p2, p3, p4, m, chi")
        file.write('\n')
        for j in range(J_n):
            for b in range(B_n):
                for t in range(T_n):
                    ...
                    text += str(probs_Arr[j, b, t, 0]) + ", " + str(probs_Arr[j, b, t, 1]) + ", " + str(probs_Arr[j, b, t, 2]) + ", " + str(probs_Arr[j, b, t, 3]) + ", "
