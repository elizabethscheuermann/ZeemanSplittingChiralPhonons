...
def main():
    ...
    Oz_Arr = np.zeros((J_n, B_n, T_n))
    probs_Arr = np.zeros((J_n, B_n, T_n, 6))
    ...
    for j in range(len(J_dom)):
        for b in range(len(B_dom)):
            for t in range(len(T_dom)):
                ...
                O = sp.optimize.minimize(F_opt, x0, args = (J_dom[j], B_dom[b]*np.sin(theta)*np.cos(phi), B_dom[b]*np.sin(theta)*np.sin(phi), B_dom[b]*np.cos(theta), T_dom[t]))
                Ox_Arr[j, b, t] = O.x[0]
                Oy_Arr[j, b, t] = O.x[1]
                Oz_Arr[j, b, t] = O.x[2]

                ### GET EIGENSTATES
                eigenvals, eigenvecs = np.linalg.eigh(H_MF(J_dom[j], O.x[0], O.x[1], O.x[2], B_dom[b]*np.sin(theta)*np.cos(phi), B_dom[b]*np.sin(theta)*np.sin(phi), B_dom[b]*np.cos(theta)))

                ### GET STATE PROBABILITIES
                for v in range(len(eigenvals)):
                    probs_Arr[j, b, t, v] = np.abs(np.exp(-eigenvals[v]/T_dom[t])/Z(T_dom[t], J_dom[j], O.x[0], O.x[1], O.x[2], B_dom[b]*np.sin(theta)*np.cos(phi),B_dom[b]*np.sin(theta)*np.sin(phi),B_dom[b]*np.cos(theta)))
    ...
    ### CREATE CSV
    with open("/home/twiggy/Research/Chiral_Phonon_Zeeman_Splitting/Data/SixLevel_Thermodynamics.csv", "w") as file:
        file.write("J, B, T, Ox, Oy, Oz, p1, p2, p3, p4, p5, p6, m, chi")
        file.write('\n')
        for j in range(J_n):
            for b in range(B_n):
                for t in range(T_n):
                    text = str(J_dom[j]) + ", " + str(B_dom[b]) + ", " + str(T_dom[t]) + ", " 
                    text += str(Ox_Arr[j,b,t]) + ", " + str(Oy_Arr[j, b, t]) + ", " + str(Oz_Arr[j,b,t]) + ", "
                    text += str(probs_Arr[j, b, t, 0]) + ", " + str(probs_Arr[j, b, t, 1]) + ", " + str(probs_Arr[j, b, t, 2]) + ", " 
                    text += str(probs_Arr[j, b, t, 3]) + ", " + str(probs_Arr[j, b, t, 4]) + ", " + str(probs_Arr[j, b, t, 5]) +", "
